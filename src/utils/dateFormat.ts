...
export const convertDateFormat = (
  ...
  if (checkYYYYMMDD(format))
    return dateFormatYYYYMMDD(year, month, day, formatSeparator);
  else if (checkMMDDYYYY(format))
    return dateFormatMMDDYYYY(year, month, day, formatSeparator);
...
export const dateFormatMMDDYYYY = (
  ...
  return `${convertToDoubleDigits(
    month
  )}${formatSeparator}${convertToDoubleDigits(day)}${formatSeparator}${year}`;
};
...
export const dateFormatDDMMYYYY = (
  ...
  return `${convertToDoubleDigits(
    day
  )}${formatSeparator}${convertToDoubleDigits(month)}${formatSeparator}${year}`;
};

const translateTitleToKo = (year: number, month: number) =>
  `${year} ${month}월`;
const translateTitleToEn = (year: number, month: number) =>
  `${months[month - 1]} ${year}`;
const translateTitleToJa = (year: number, month: number) =>
  `${year}年 ${month}月`;
...
export const convertTitleToUnit = (
  ...
  if (unit === EUnit.DECADE)
    return `${year}-${year < 0 ? year - 90 : year + 90}`;
...
export const checkYYYYMMDD = (format: string) =>
  format.startsWith("Y") || format.startsWith("y");
export const checkDDMMYYYY = (format: string) =>
  format.startsWith("D") || format.startsWith("d");
export const checkMMDDYYYY = (format: string) =>
...
export const findYearInStr = (
  date: string,
  format: string = "YYYY-MM-DD"
): number => {
  if (checkYYYYMMDD(format))
    return +date.split(findSpecialCharacterStr(format))[0];
  else return +date.split(findSpecialCharacterStr(format))[2];
};

export const findMonthInStr = (
  date: string,
  format: string = "YYYY-MM-DD"
): number => {
  if (checkMMDDYYYY(format))
    return +date.split(findSpecialCharacterStr(format))[0];
  else return +date.split(findSpecialCharacterStr(format))[1];
};

export const findDayInStr = (
  date: string,
  format: string = "YYYY-MM-DD"
): number => {
  if (checkYYYYMMDD(format))
    return +date.split(findSpecialCharacterStr(format))[2];
  else if (checkMMDDYYYY(format))
    return +date.split(findSpecialCharacterStr(format))[1];
...
export const convertToDeafultFormat = (
  value: string,
  format: string
): string => {
  ...
  if (checkDDMMYYYY(format)) {
    ...
    DataFormat = `$3${findSpecialCharacterStr(
      value
    )}$2${findSpecialCharacterStr(value)}$1`;
  } else {
    RegDateFmt = /([0-9]{2})([0-9]{2})([0-9]+)/;
    DataFormat = `$3${findSpecialCharacterStr(
      value
    )}$1${findSpecialCharacterStr(value)}$2`;
  }
